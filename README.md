## Ansible主机巡检生成网页报告

## 项目地址
```bash
https://github.com/gpjdean/AnsibleHealthCheck.git
```


## 功能特点：
⚡自动化巡检： 通过 Ansible Playbook 编排巡检任务，实现一键自动化巡检，大大提高运维效率。

📊多维度检查： 支持 CPU、内存、磁盘、网络、服务等多个维度的检查，全面掌握服务器运行状况。

🔧灵活扩展： 基于 Ansible 的模块化设计，可以轻松扩展自定义检查项，满足个性化需求。

🖥️可视化报告： 生成 HTML 格式的巡检报告，直观展示各项数据结果。

📡集中化管理： 通过 Ansible Tower 或 AWX 等平台，实现集中化巡检管理，提高运维效率。

## 使用场景：
✅日常巡检： 定期执行巡检任务，及时发现潜在问题，保障服务器稳定运行。

🔍故障排查： 通过巡检报告快速定位问题，缩短故障恢复时间。

👥批量管理： 适用于大规模服务器集群的巡检，提高运维效率。

📧报告归档： 自动将巡检报告发送至邮箱，实现远程监控与历史数据归档，便于后续分析。

## 项目优势：
⏱️高效节省人力： 自动化采集与报告生成大大降低了手工巡检的工作量，释放运维人员精力。

📈数据标准化： 统一的报告模板保证了巡检数据的格式一致，便于比对与趋势分析。

🚨实时预警： 报告中可通过颜色高亮标注异常状态，帮助运维人员第一时间发现问题。

⚙️易部署维护： 基于 Ansible 的解决方案部署简单，只需修改少量配置变量即可适应不同环境；同时支持灵活扩展巡检项。

🌐多种查看方式： 报告既支持邮件直达，也可通过浏览器在线预览，满足多种运维需求。

✨简单易用： 基于 Ansible 的声明式语法，易于上手，无需编写复杂脚本。

🆓开源免费： 项目开源免费，您可以自由使用和修改。



## 安装Ansible
```bash
yum -y install epel-release
yum -y install ansible
ansible --version
```



## 克隆代码
```bash
[root@instance-euwvmd1u ~]# git clone https://ghfast.top/https://github.com/gpjdean/AnsibleHealthCheck.git
Cloning into 'ansible-HealthCheck'...
remote: Enumerating objects: 40, done.
remote: Counting objects: 100% (40/40), done.
remote: Compressing objects: 100% (26/26), done.
remote: Total 40 (delta 12), reused 35 (delta 7), pack-reused 0 (from 0)
Unpacking objects: 100% (40/40), done.
```



## 修改项目ansible.cfg配置
```bash
# 进入到项目路径
[root@instance-euwvmd1u ~]# cd AnsibleHealthCheck

# 指空文件内容
[root@instance-euwvmd1u AnsibleHealthCheck]# >ansible.cfg 

# 编辑配置文件，将下面的内容写入到ansible.cfg文件内
[root@instance-euwvmd1u AnsibleHealthCheck]# vim ansible.cfg

[defaults]
host_key_checking=False
remote_user=root
filter_plugins=/root/AnsibleHealthCheck/filter_plugins
[inventory]
[privilege_escalation]
[paramiko_connection]
[ssh_connection]
[persistent_connection]
[accelerate]
[selinux]
[colors]
[diff]
```


## 配置hosts主机信息
```bash
[root@instance-euwvmd1u AnsibleHealthCheck]# vim hosts
# 内容如下
[k8s]
120.48.168.44
[k8s:vars]
ansible_ssh_user="root"
ansible_ssh_pass="你的主机密码"
```

## 报告及邮箱配置【可选】
```bash
vim /root/AnsibleHealthCheck/os-check/defaults/main.yaml，修改以下内容：
报告目录：check_report_path	设置报告生成目录（例如 /tmp）
SMTP相关变量（check_mail_host、check_mail_port、check_mail_username、check_mail_password、check_mail_to）根据实际情况填写
```

## 执行剧本，进行巡检
```bash
[root@instance-euwvmd1u AnsibleHealthCheck]# ansible-playbook -i hosts roles/os-check.yaml --limit k8s

PLAY [k8s] **********************************************************************************************************************************************************************************

TASK [os-check : Get system check data.] ****************************************************************************************************************************************************
changed: [120.48.168.44]

TASK [os-check : Generate report file.] *****************************************************************************************************************************************************
changed: [120.48.168.44]

TASK [os-check : Get report file content.] **************************************************************************************************************************************************
ok: [120.48.168.44]

TASK [os-check : Send a report by email.] ***************************************************************************************************************************************************
skipping: [120.48.168.44]

PLAY RECAP **********************************************************************************************************************************************************************************
120.48.168.44              : ok=3    changed=2    unreachable=0    failed=0    skipped=1    rescued=0    ignored=0 
```



## 创建一个http服务实现Web访问
```bash
# 前台执行
[root@instance-euwvmd1u AnsibleHealthCheck]# cd /tmp/ && python -m SimpleHTTPServer 30000
Serving HTTP on 0.0.0.0 port 30000 ...

# 后台运行
[root@instance-euwvmd1u AnsibleHealthCheck]# cd /tmp/ && nohup python -m SimpleHTTPServer 30000 >/dev/null 2>&1 &
```


## 浏览器访问页面
```bash
http://120.48.168.44:30000/report-2025-xx-xx.html
```

## 效果图
<img width="2559" height="590" alt="image" src="https://github.com/user-attachments/assets/81470925-8cdf-4094-b571-59f352509a06" />

